...
export const fetchProductById = async (
  ...
  try {
    const response = await fetch(config.product.getProductById(productId),{   next: { revalidate: 30 } });
    if (!response.ok) {
      return null;
    }
...
export const fetchAllProducts = async (
  ...
  try {
    const response = await fetch(`${config.product.getProducts()}?limit=${500}`,{   next: { revalidate: 30 } });
    if (!response.ok) {
      return [];
    }
    const productData = await response.json();
